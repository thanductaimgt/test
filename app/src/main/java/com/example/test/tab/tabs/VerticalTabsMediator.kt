package com.example.test.tab.tabs
 ...
 *
 */
class VerticalTabsMediator(
    private val tabs: VerticalTabs,
    ...
    interface TabConfigurationStrategy {
         ...
         */
        fun onConfigureTab(tabView: TabView, position: Int)
    }
    ...
    ) : this(tabs, viewPager, true, tabConfigurationStrategy)
    ...
    constructor(
        ...
        autoRefresh: Boolean,
        tabConfigurationStrategy: TabConfigurationStrategy
    ) : this(tabs, viewPager, autoRefresh, true, tabConfigurationStrategy)
    ...
    fun attach() {
        ...
        if (autoRefresh) {
            ...
            adapter?.registerAdapterDataObserver(PagerAdapterObserver().also {
                pagerAdapterObserver = it
            })
        }
    ...
    fun populateTabsFromPagerAdapter() {
        ...
        if (adapter != null) {
            ...
            for (i in 0 until adapterCount) {
                val tab = QTabView(tabs.context)
                tabConfigurationStrategy.onConfigureTab(tab, i)
                tabs.addTab(tab, false, true)
            }
            // Make sure we reflect the currently set ViewPager item
            if (adapterCount > 0) {
                val lastItem = tabs.tabCount - 1
                val currItem = viewPager.currentItem.coerceAtMost(lastItem)
                if (currItem != tabs.selectedTabPosition) {
                    tabs.setTabSelected(currItem)
                }
            }
        }
    }
     ...
     */
    private class TabLayoutOnPageChangeCallback(tabs: VerticalTabs) :
        OnPageChangeCallback() {
        private val tabsRef: WeakReference<VerticalTabs> = WeakReference(tabs)
        ...
        override fun onPageScrolled(
            ...
            if (tabs != null) {
                ...
                tabs.setScrollPosition(position, positionOffset, updateText, updateIndicator)
            }
        }

        override fun onPageSelected(position: Int) {
            val tabs = tabsRef.get()
            if (tabs != null && tabs.selectedTabPosition != position && position < tabs.tabCount) {
...
//                        && previousScrollState == ViewPager2.SCROLL_STATE_IDLE))
                tabs.setTabSelected(position, false, true)
            }
        }
